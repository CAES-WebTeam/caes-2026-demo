...
| Locations / global map | | Reuse the `uga-caes/locations` family (`all-locations-map`, `single-location-map`, + `list-location-tag`/`list-location-category`/`location-address` helpers). CPT-driven: reads a `location` CPT (tax `location-category`/`location-tag`, ACF `latitude`/`longitude`) via REST `/wp-json/wp/v2/location`; renders clustered GeoJSON on **MapLibre GL**. Intended reuse: our **Global Programs Map**. **ISSUE: the `location` CPT is NOT registered in caes-blocks** (only `caes_events` is) — it appears to live in a separate `caes-locations` plugin (marker-asset path references it). Decide: bring the CPT registration into caes-blocks (theme-agnostic) or point the block at a new programs-location CPT/source. Also: MapLibre is loaded from `unpkg.com` CDN in `render.php` — consider bundling. | Existing block updates | MOD-locations |
...
## Decisions log
_Newest first._
- Added MOD-locations (reuse `uga-caes/locations` map family for the Global Programs Map). Open issue: the `location` CPT isn't registered in caes-blocks (lives in a separate `caes-locations` plugin) — decide where the CPT/source comes from.
