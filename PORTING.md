...
| Shared style primitives | Ashley | Theme-owned classes used across many pieces — blocks/patterns **emit** the class, never restyle. Card look (`.card`), stretched link (`.card-link`), fade-up reveal, button variations (`.btn*` → core Button styles), section type (`.eyebrow`/`.section-kicker`/`.section-display`/`.section-lead`), `.link-arrow`, `.red-rule`. Standardize the card/reveal utilities from the per-section demo CSS. Build early — dependency of every card/section consumer. Full table in reference notes. | Theme Setup | STY |
| Dept promo button style | Ashley | Red, full-width, trailing-arrow Button style variation (core Button) for the Connect dept-promo link. Source: `connect` (`connect-dept-card`). | Theme Setup | THM-btn-promo |
...
| Degrees & Programs | Ashley | Archive template for the `program` CPT; searchable/filterable list (programs listing block — new or Query-based, TBD). Depends on Program CPT. Source: `programs`. | Templates | TPL-programs-archive |
...
| Announcement bar | | Static pattern (place in a template part). Dismiss behavior dropped (would need `view.js`/cookie + a block). Source: `announcement`. | Patterns | PAT-announce |
...
| Brand / logo lockup | | New `uga-caes/brand` — CAES lockup for all sites. Bundled logo enum (CAES, UGA wordmark; Extension later); not media library. Variant attr (light/dark/mono) via `--wp--preset--color--*` tokens. Template-part contexts only. Folds in the home-link wrapper. Build before PART-header/PART-footer. Prior art: `caes-faculty-2023`. Source: `_shared.jsx`. | Plugin blocks (new) | BLK-brand |
...
| UGA footer | | New `uga-caes/uga-footer` — reused across many UGA/CAES sites (plugin, not per-theme). Migrate/copy the `caes-faculty-2023` footer part into the block. Arch-bg SVG. Build before PART-footer. Source: `uga-footer`. | Plugin blocks (new) | BLK-uga-footer |
...
| Taxonomies | | **Program type** (Major/Minor/Certificate/Graduate — drives header eyebrow + archive filtering); **Degree awarded** (store abbreviation + written-out name, e.g. B.S.A. → Bachelor of Science in Agriculture); **Department** (terms carry a URL term-meta = dept site link, feeds the Connect promo link); **Campus** (Athens/Tifton/Griffin, filterable). | Program CPT | CPT-tax |
| Meta fields | | **Summary/lead** (textarea or excerpt); **Apply URL** + **Visit URL** (global default + optional per-program override); **How to prepare / Hands-on learning / Career roles** (repeatable text, stored on CPT so reusable); **Career outcomes** (repeatable label + %, drives outcome bars) + **source/footnote**; **Double Dawgs eligible** (boolean toggle); **FAQ** (repeatable Q + A). About/Coursework narrative stays in post content. Dept promo link is derived from the Department term URL — not its own field. | Program CPT | CPT-meta |
| Relationships | | **Advisors** → personnel (relationship field; program stores references; rendered as cards linking to Field Report via MOD-personnel — no individual profile pages). **Double Dawgs pairings** → program (program-to-program relationship to the paired master's posts). | Program CPT | CPT-rel |
...
- **Program CPT before** TPL-programs-archive, TPL-program-single, CPT-header (the header pattern).
